 ...
 * @version 1.0
 * <p>
 * Description: 把字符串转换成整数 (atoi),请你来实现一个 myAtoi(string s) 函数，使其能将字符串转换成一个 32 位有符号整数（类似 C/C++ 中的 atoi 函数）。
 * 函数 myAtoi(string s) 的算法如下：  读入字符串并丢弃无用的前导空格 检查下一个字符（假设还未到字符末尾）为正还是负号，读取该字符（如果有）。
...
public class LCR192_MyAtoi {
    ...
    public static void main(String[] args) {
        LCR192_MyAtoi myAtoi = new LCR192_MyAtoi();
